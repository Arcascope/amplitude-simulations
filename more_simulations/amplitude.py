...
def generate_light_dark_schedule(light_hours, period, brightness, dt=0.1, cycles_needed=200, want_blur=True):
    ...
    if want_blur:
        width = 1 / dt  # One hour width blur
        schedule = gaussian_filter(schedule, sigma=width)
    time_array = np.arange(0, cycle_duration * cycles_needed, dt)

    plt.close()
    plt.plot(time_array[:int(96 / dt)], schedule[:int(96 / dt)])
    plt.savefig(f"debug/{light_hours}_{period}_{brightness}.png")
    plt.close()
    ...
    for WANT_AVERAGE in [True]:
        ...
        for i, light_hours in enumerate(light_hours_range):
            ...
            for j, brightness in enumerate(brightness_levels):
                ...
                initial_state = np.array([1, np.pi, 0.519, 0.0, 0.0])
        ...
        X, Y = np.meshgrid(range(len(brightness_levels)),
                           light_hours_range)  # Use index positions for brightness levels
        ...
        for i, period in enumerate(period_duration):
            ...
            for j, brightness in enumerate(brightness_levels):
                time_array, schedule = generate_light_dark_schedule(int(2 * period / 3), period, brightness, dt=dt,
                                                                    cycles_needed=int(50 * 24 / period),
                                                                    want_blur=False)
                initial_state = np.array([1, np.pi, 0.519, 0.0, 0.0])
        ...
        plt.ylabel("Period Duration (hours)")
        metric = "Average" if WANT_AVERAGE else "Max"
        plt.title(f"Brightness and day length (2 L : 1 D)\nEffects on {metric} Amplitude")
        plt.savefig(f"Brightness and day length (2 L to 1 D) - Effects on {metric} Amplitude.png", dpi=300)
